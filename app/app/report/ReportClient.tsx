...
function Stat({ label, value, sub }: { label: string; value: React.ReactNode; sub?: string }) {
  return <div className="card flex flex-col gap-1 p-4"><span className="text-[12px] uppercase tracking-[0.08em] text-ink-muted">{label}</span><span className="num text-[28px] font-semibold leading-none">{value}</span>{sub && <span className="text-[13px] text-ink-muted">{sub}</span>}</div>;
}
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px] px-4 py-8 md:px-6">
      <div className="flex flex-wrap items-end gap-3">
        <div>
          <p className="mono text-[12px] uppercase tracking-[0.08em] text-ink-muted">After-action report · {report.town} · zone {report.zone}</p>
      ...
      <section className="mt-8" aria-labelledby="proof">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <Stat label="Reached" value={pct(report.reachRate)} sub={`${report.reached} answered, ${counts.NO_ANSWER} never picked up`} />
          <Stat label="Time to every first attempt" value={minutes(metrics.minutesToFirstAttempt)} sub="8 lines in parallel, simulated calls" />
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-[1fr_320px]">
            <div className="card overflow-x-auto p-4">
              <table className="w-full text-[14px]">
                ...
                <tbody>{LABELS.map((row) => <tr key={row} className="border-t border-line"><td className="py-2 font-semibold">{row}</td>{LABELS.map((col) => { const v = evaluation.matrix?.[row]?.[col] || 0; const unsafe = (row === "URGENT" || row === "UNSURE") && col === "OK" && v > 0; return <td key={col} className={`num py-2 pr-3 text-right ${row === col ? "text-success" : unsafe ? "text-danger" : v ? "text-warning" : "text-ink-muted"}`}>{v}</td>; })}</tr>)}</tbody>
              </table>
            </div>
