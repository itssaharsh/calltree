...
for (const [route, qs] of shots) for (const w of widths) {
  ...
  await p.goto(`${base}${route}${qs ? '?' + qs : ''}`, { waitUntil: 'networkidle' }).catch((e) => errors.push(`${route}: ${e.message}`));
  ...
  await p.screenshot({ path: new URL(`../../qa/${name}`, import.meta.url).pathname, fullPage: w !== 1440 || route !== '/ops/' });
  await p.close();
}
await b.close();
